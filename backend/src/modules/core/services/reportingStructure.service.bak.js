...
// ---------------------------------------------------------------------------
// Simple in-memory cache for the hierarchy tree (avoids repeat 5 s queries)
// ---------------------------------------------------------------------------
const _treeCache = { data: null, ts: 0, TTL: 60_000 }; // 60-second TTL
...
// Invalidate on module load so any previous stale cache is cleared on restart
invalidateTreeCache();
...
async function assignManagerChain(userId, managerChain, createdById) {
  ...
  // After the loop, currentEmployee is the top-most manager in the chain.
  // The depth system counts from the bottom: employee = 1, L1 manager = 2, ...
  // So the top manager's depth = managerChain.length + 1
  // e.g.  Nisha(1) → Rahul(2) → Shruti(3)
  const topManager = currentEmployee;
  const topManagerDepth = managerChain.length + 1;
  await prisma.reportingStructure.upsert({
    where: { userId: topManager },
    update: {
      managerId: null,
      hierarchyDepth: topManagerDepth,
      hierarchyPath: topManager,
      updatedAt: new Date()
    },
    create: {
      userId: topManager,
  ...
  createdRelationships.push({ userId: topManager, managerId: null, hierarchyDepth: topManagerDepth });
...
/**
 * Delete a user's reporting record and re-parent their direct subordinates
 * to the deleted user's own manager (or null if they were a root node).
 *
 * @param {string} userId - User whose reporting record to delete
 */
async function deleteReportingRelationship(userId) {
  ...
  const parentManagerId = userReporting.managerId; // null when user is a root node

  // Collect direct subordinates before deleting
  const subordinates = await prisma.reportingStructure.findMany({
    where: { managerId: userId },
  });

  // Re-parent subordinates to grandparent (or promote to root)
  if (subordinates.length > 0) {
    await prisma.reportingStructure.updateMany({
      where: { managerId: userId },
      data: { managerId: parentManagerId ?? null },
    });
  }

  // Remove this user's reporting record
  await prisma.reportingStructure.delete({ where: { userId } });

  // Refresh depth/path for the affected branch
  if (parentManagerId) {
    await updateSubordinatesHierarchy(parentManagerId);
  } else if (subordinates.length > 0) {
    // Promoted to root — reset depth and path for each
    for (const sub of subordinates) {
      await prisma.reportingStructure.update({
        where: { userId: sub.userId },
        data: { hierarchyDepth: 0, hierarchyPath: sub.userId },
      });
      await updateSubordinatesHierarchy(sub.userId);
    }
  }
...
async function getHierarchyTree() {
  ...
  const now = Date.now();
  if (_treeCache.data && (now - _treeCache.ts) < _treeCache.TTL) {
 ...
 * hierarchyDepth is recomputed from the ACTUAL parent-child relationships
 * (leaf node = 1, each level above +1) so stale DB values never cause
 * display mismatches.
...
function buildTree(reportingList) {
  ...
  // Create nodes (hierarchyDepth will be overwritten below)
  reportingList.forEach(r => {
    nodeMap.set(r.userId, {
      ...
      hierarchyDepth: r.hierarchyDepth, // temp – recalculated below
      children: []
    });
  });
  ...
  reportingList.forEach(r => {
    ...
    if (r.managerId && nodeMap.has(r.managerId)) {
      ...
      roots.push(node); // No manager (or manager not in tree) = root
    }
  });

  // Recompute hierarchyDepth from actual tree structure so stale DB
  // values never cause wrong Level badges.
  // Rule: leaf node = Level 1, each node above its deepest child = +1.
  function computeDepth(node) {
    if (!node.children || node.children.length === 0) {
      node.hierarchyDepth = 1;
      return 1;
    }
    let maxChildDepth = 0;
    ...
    node.hierarchyDepth = maxChildDepth + 1;
    return node.hierarchyDepth;
  }

  roots.forEach(root => computeDepth(root));
