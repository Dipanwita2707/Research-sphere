/**
 * Bug Report System - Endpoint Testing Script with Automatic Authentication
 * 
...
function createTestScreenshot(filename, sizeKB = 100) {
  const buffer = Buffer.alloc(sizeKB * 1024, 'test data for screenshot');
...
async function loginFinanceUser() {
  ...
  try {
    const response = await axios.post(`${BASE_URL}/api/v1/auth/login`, {
      uid: FINANCE_USER_CREDENTIALS.uid,
...
async function runTests() {
  ...
  console.log(`${colors.blue}with Automatic Authentication${colors.reset}`);
