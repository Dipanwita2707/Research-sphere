...
function FieldRow({
  ...
  return (
    <div className="py-3 first:pt-0">
      <label className="flex items-center gap-1.5 text-[11px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1.5">
...
function PersonCard({
  ...
  return (
    <div className="flex items-center gap-3 px-3 py-2.5 bg-gray-50 dark:bg-gray-700/50 border border-gray-200 dark:border-gray-600 rounded-lg">
      ...
      <span className="text-[10px] bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 px-2 py-0.5 rounded-full font-medium flex-shrink-0 whitespace-nowrap">
        {role}
      </span>
    </div>
  );
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-5">
        ...
        <div className="space-y-0 divide-y divide-gray-100 dark:divide-gray-700/50">
          ...
          <FieldRow label="Club Category">
            {resolvedDetails?.categoryName ? (
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-medium bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800">
                  {resolvedDetails.categoryName}
                </span>
              </div>
            ) : data.clubCategoryId ? (
          ...
          <FieldRow label="Purpose / Objective">
            {data.clubPurpose ? (
              <div className="bg-gray-50 dark:bg-gray-900/20 px-3 py-2.5 rounded-md border border-gray-100 dark:border-gray-700">
          ...
          <FieldRow label="Academic Session" icon={Calendar}>
            {data.clubAcademicSession ? (
              <span className="inline-flex items-center px-3 py-1 rounded-md text-sm font-medium bg-amber-50 dark:bg-amber-900/20 text-amber-800 dark:text-amber-200 border border-amber-200 dark:border-amber-800">
                {data.clubAcademicSession}
              </span>
            ) : (
              <NotProvided />
            ...
            {resolvedDetails?.members && resolvedDetails.members.length > 0 ? (
              <div>
                <p className="text-xs text-gray-500 dark:text-gray-400 mb-2">
                  <span className="font-semibold text-gray-700 dark:text-gray-300">
                    {resolvedDetails.members.length}
                  </span>{" "}
                ...
                <div className="flex flex-wrap gap-1.5">
                  {resolvedDetails.members.map((m) => (
                    <span
                      key={m.id}
                      className="inline-flex items-center gap-1 px-2 py-1 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-full text-xs text-blue-800 dark:text-blue-200"
                      title={`${m.name} (${m.uid})`}
                    >
                      <span className="w-4 h-4 rounded-full bg-blue-200 dark:bg-blue-800 flex items-center justify-center text-[9px] font-bold text-blue-700 dark:text-blue-300 flex-shrink-0">
              ...
              <div className="flex flex-wrap gap-1.5">
                {data.clubInitialMembers.map((uid, i) => (
                  <span
                    key={i}
                    className="inline-flex items-center px-2 py-0.5 bg-gray-100 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-full text-xs text-gray-700 dark:text-gray-300"
                  >
                    {uid}
                  </span>
                ))}
                    ...
                    <span
                      ...
                      className={`inline-flex items-center px-3 py-1.5 rounded-lg text-xs font-medium border-2 transition-colors ${
                        isSelected
                          ...
                          : "border-gray-200 dark:border-gray-600 text-gray-400 dark:text-gray-500 bg-gray-50 dark:bg-gray-800"
                      }`}
                    >
                      {isSelected && (
      ...
      <div className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-5">
        ...
        <div className="space-y-0 divide-y divide-gray-100 dark:divide-gray-700/50">
          {/* Declarations */}
          <div className="py-3 first:pt-0">
            <label className="text-[11px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-3 block">
              Required Declarations
            </label>
            <div className="space-y-3">
              <div className="flex items-start gap-3 px-3 py-2.5 bg-gray-50 dark:bg-gray-700/30 rounded-lg border border-gray-100 dark:border-gray-700">
              ...
              <div className="flex items-start gap-3 px-3 py-2.5 bg-gray-50 dark:bg-gray-700/30 rounded-lg border border-gray-100 dark:border-gray-700">
                <BooleanIndicator
                  value={data.clubAntiDiscriminationAccepted}
                />
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-gray-900 dark:text-white">
              ...
              <div className="grid grid-cols-2 gap-2">
                {(
                  ...
                  return (
                    <div
                      key={key}
                      className="flex items-center gap-2 px-2.5 py-1.5 bg-gray-50 dark:bg-gray-700/30 rounded-md border border-gray-100 dark:border-gray-700"
                    >
                      <span className="text-[10px] font-semibold text-gray-400 uppercase w-16 flex-shrink-0">
                        {label}
                      </span>
