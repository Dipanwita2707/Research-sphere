...
export default function IPRIdeaRequestForm({ initialType = 'patent' }: IPRIdeaRequestFormProps) {
  ...
  const isCurrentUserStudent = user?.userType ===
   'student';
  ...
  const addContributor = () => {
   ...
   'internal' && (!formData.uid || !formData.name)) {
      setError('Please fill UID and Name for internal contributors');
      return;
    }
    if (formData.employeeCategory ===
   'external' && (!formData.externalName || !formData.externalEmail)) {
   ...
   formData.externalEmail;
    ...
    const newContributor = {
      ...
      uid: formData.uid,
      name: formData.employeeCategory ===
   'internal' ? formData.name : formData.externalName,
      email: formData.employeeCategory ===
   'internal' ? formData.email : formData.externalEmail,
      phone: formData.employeeCategory ===
   'internal' ? formData.phone : formData.externalPhone,
  ...
  const calculateContributorIncentivePoints = (employeeCategory: string, employeeType: string) => {
    ...
    if (currentPolicy?.baseIncentiveAmount !== undefined && currentPolicy?.baseIncentiveAmount !== null) {
      totalBaseIncentive = typeof currentPolicy.baseIncentiveAmount ===
   'string' 
   ...
   'trademark' ? 15 : 20;
    ...
    const eligibleForIncentive = contributors.filter(c => c.employeeCategory ===
   'internal').length;
    ...
    // Note: Applicant is NOT included in the count - only added contributors are counted
    const eligibleForPoints = contributors.filter(c => c.employeeCategory ===
  ...
  const handleSubmit = async (e: React.FormEvent) => {
   ...
   'complete') {
      ...
      if (formData.completeFilingSource ===
   'from_provisional' && !formData.sourceProvisionalId) {
    ...
    // Consent validation
    if (!consentChecked) {
      errors.push('Please confirm that the work has been done at ResearchSphere');
    }
    ...
    try {
      ...
      if (formData.typeOfFiling ===
   'complete' && formData.prototypeFile) {
      ...
      const applicationData = {
        ...
        sdgs: formData.sdg.map(code => ({
          code,
          title: SDG_OPTIONS.find(s => s.value ===
   code)?.label || ''
        })),
        
        applicantDetails: formData.employeeCategory ===
   'internal' ? {
        ...
        ...(formData.typeOfFiling ===
   'complete' && {
          sourceProvisionalId: formData.completeFilingSource ===
   'from_provisional' ? formData.sourceProvisionalId : undefined,
  ...
  return (
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-gray-500 dark:text-gray-400 text-sm">ResearchSphere</p>
              </div>
            </div>
   ...
   'entry' && (
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-4 gap-6">
              ...
              {formData.typeOfFiling ===
   'complete' && (
                <div className="col-span-2 space-y-4 p-4 bg-[#fdf5ec] dark:bg-[#7d1a34]/10 border border-[#f0e2d2] dark:border-[#5e1024] rounded-lg">
                  <h4 className="font-medium text-[#7d1a34] dark:text-[#c8973f] flex items-center gap-2">
                    ...
                    <div className="flex gap-4">
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input
                          ...
                          value="fresh"
                          checked={formData.completeFilingSource ===
   'fresh'}
                          onChange={handleInputChange}
                          className="text-[#7d1a34]"
                        />
   ...
   'fresh' && <span className="text-red-500">*</span>}
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        className="flex-1 text-sm file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-[#fdf5ec] file:text-[#7d1a34] hover:file:bg-[#fbe2e8]"
                      />
                    ...
                    {prototypeUploading && (
                      <p className="text-sm text-[#7d1a34] mt-1 flex items-center gap-1">
            ...
            <div className="grid grid-cols-4 gap-6">
              ...
              <div>
                ...
                <div className="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-md p-4 text-center">
                  <button type="button" className="bg-[#7d1a34] text-white px-4 py-2 rounded-md text-sm mb-2">
                  ...
                  <input
                    ...
                    accept={formData.ideaFor ===
   'patent' ? '.doc,.docx,.zip,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/zip,application/x-zip-compressed' : '.zip,.pdf,.doc,.docx'}
                        ...
                        <div className="absolute top-full left-0 right-0 mt-1 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md shadow-lg z-20 max-h-48 overflow-y-auto">
                          {uidSuggestions.map((suggestion, index) => (
                            <div
                              ...
                              className="px-3 py-2 hover:bg-[#fdf5ec] dark:hover:bg-[#7d1a34]/20 cursor-pointer border-b border-gray-100 dark:border-gray-700 last:border-b-0"
                            >
   ...
   'internal' && contributor.employeeType !== 'student' ? (
                                  <span className="text-[#7d1a34] font-medium">{points.toFixed(2)}</span>
                                ) : contributor.employeeCategory ===
   'internal' && contributor.employeeType ===
   'student' ? (
   ...
   'complete' ? 'bg-[#fdf5ec] border border-[#f0e2d2] dark:bg-[#7d1a34]/10 dark:border-[#5e1024]' : 'bg-gray-50 border border-gray-200 dark:bg-gray-800 dark:border-gray-700'}`}>
                {formData.typeOfFiling ===
   'complete' ? (
                  <div className="flex items-start">
                    <AlertCircle className="w-5 h-5 text-[#7d1a34] mr-2 mt-0.5" />
                    <div>
                      <p className="text-sm font-medium text-[#7d1a34] dark:text-[#c8973f]">Complete Filing Selected</p>
                      <p className="text-xs text-[#7d1a34] dark:text-[#c8973f] mt-1">
                        {isCurrentUserStudent 
              ...
              <div className="mb-6 p-4 bg-yellow-50 border border-yellow-200 dark:bg-yellow-900/20 dark:border-yellow-800 rounded-md">
                <label className="flex items-start">
                  ...
                  <span className="text-sm text-gray-700 dark:text-gray-300">
                    I here by confirm that the work has been done in ResearchSphere and I give my full consent for the filing of Intellectual Property Rights from ResearchSphere.
                  </span>
                </label>
              </div>
   ...
   'process' && (
          <div className="bg-white dark:bg-gray-900">
            ...
            {(() => {
              ...
              const contributedApps = contributedApplications.filter(app => app && app.id && !applications.some(ownApp => ownApp.id ===
   app.id));
              ...
              return (
                <div className="mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
                  {/* Own IPR Stats */}
                  <div className="bg-[#fdf5ec] border border-[#f0e2d2] dark:bg-[#7d1a34]/10 dark:border-[#5e1024] rounded-lg p-4">
                    <h3 className="text-sm font-semibold text-[#7d1a34] dark:text-[#c8973f] mb-2">My IPR Applications</h3>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-600 dark:text-gray-400">Total: {ownApps.length}</span>
                      <span className="text-[#7d1a34] dark:text-[#c8973f]">Published: {ownApps.filter(a => publishedStatuses.includes(a.status)).length}</span>
                    </div>
                    <div className="mt-2 pt-2 border-t border-[#f0e2d2] dark:border-[#5e1024]">
                      <div className="text-xs text-gray-600 dark:text-gray-400">Points Earned: <span className="font-bold text-[#7d1a34] dark:text-[#c8973f]">{ownTotalPoints}</span></div>
                ...
                <tbody>
                  {(() => {
                    ...
                    const contributedApps = contributedApplications
                      .filter(app => app && app.id && !applications.some(ownApp => ownApp.id ===
   app.id))
   ...
   0) {
                    ...
                    return allApplications.map((app, index) => (
                      <tr key={app.id} className={`hover:bg-gray-50 dark:hover:bg-gray-700/50 ${app.isContributor ? 'bg-purple-50 dark:bg-purple-900/10' : 'dark:bg-gray-800'}`}>
                        ...
                        <td className="px-4 py-2 border dark:border-gray-600 text-sm">
                          <span className="inline-flex items-center px-2 py-1 bg-[#fbe2e8] text-[#7d1a34] text-xs rounded uppercase font-medium">
                            {app.iprType}
                          </span>
   ...
   'draft' ? 'bg-gray-100 text-gray-800' :
                            app.status ===
   'pending_mentor_approval' ? 'bg-indigo-100 text-indigo-800' :
                            app.status ===
   'submitted' ? 'bg-[#fbe2e8] text-[#7d1a34]' :
                            app.status ===
   'under_drd_review' ? 'bg-yellow-100 text-yellow-800' :
                            app.status ===
   'drd_approved' ? 'bg-green-100 text-green-800' :
                            app.status ===
   'under_dean_review' ? 'bg-purple-100 text-purple-800' :
                            app.status ===
   ...
   'changes_required' ? 'bg-orange-100 text-orange-800' :
                        ...
                        <td className="px-4 py-2 border dark:border-gray-600 text-sm text-center">
                          {app.isContributor ? (
                            ...
                            <span className="inline-flex items-center px-2 py-1 bg-[#fbe2e8] text-[#7d1a34] text-xs rounded-full font-medium">
                              👤 Applicant
                            </span>
                          )}
                        </td>
   ...
   'draft' && (
                              <button
                                ...
                                className="bg-[#7d1a34] text-white px-3 py-1 rounded-lg text-xs hover:bg-[#5e1024] transition-colors w-full"
                              >
