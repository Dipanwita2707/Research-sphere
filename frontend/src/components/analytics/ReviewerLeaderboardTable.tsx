...
export default function ReviewerLeaderboardTable({ reviewers, onReviewerClick, selfView }: Props) {
  ...
  const SortHeader = ({ label, field }: { label: string; field: SortKey }) => (
    <th
      className="px-4 py-3 font-medium text-gray-500 cursor-pointer hover:text-gray-700 select-none text-right"
      onClick={() => handleSort(field)}
    >
      <span className="inline-flex items-center gap-1">
        {label}
        {sortKey ===
   field && <span className="text-xs">{sortAsc ? '↑' : '↓'}</span>}
      </span>
    </th>
  );
  ...
  if (!reviewers.length) {
    return (
      <div className="rounded-[24px] border border-white/70 bg-white/85 p-8 text-center text-slate-400 shadow-[0_16px_36px_-24px_rgba(15,23,42,0.22)] backdrop-blur-sm">
  ...
  return (
    <div className="overflow-hidden rounded-[28px] border border-white/70 bg-white/85 shadow-[0_18px_44px_-24px_rgba(15,23,42,0.24)] backdrop-blur-xl">
      <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
        ...
        <span className="text-xs text-slate-400">{reviewers.length} reviewer{reviewers.length !== 1 ? 's' : ''}</span>
      </div>
              ...
              return (
                ...
                >
                  <td className="px-4 py-3">
                    <div className="font-medium text-slate-900">{r.reviewerName}</div>
                    {r.lastActiveAt && (
                  ...
                  <td className="px-4 py-3 text-right font-medium text-slate-800">{r.assigned}</td>
                    ...
                    <div className="flex items-center gap-1 justify-center">
                      {total > 0 && (
                        <>
                          ...
                          <span className="ml-1 text-xs text-slate-400">{approvedPct}%</span>
                        </>
                      )}
                    </div>
                  </td>
