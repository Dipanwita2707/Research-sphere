...
import { ArrowLeft, Plus, Trash2, Upload, FileText, GripVertical, Clock, CheckCircle, IndianRupee, User, Send, Save, Paperclip, AlertCircle, List, Calendar } from 'lucide-react';
...
export default function NewNotePage() {
  ...
  const [eventResources, setEventResources] = useState<Array<{ category: 'internal' | 'external'; type: string; description: string; estimatedCost?: number }>>([]);
  ...
  useEffect(() => {
    ...
    eventSponsors, eventHasResources, eventResources, setDraftId,
  ]);
                    ...
                    {eventHasResources && (
                      <div className="mt-2 space-y-2">
                        {eventResources.map((r, i) => (
                          <div key={i} className="flex flex-wrap gap-2 p-2 rounded-md border border-gray-200 dark:border-gray-600">
                            <select value={r.category} onChange={(e) => setEventResources((prev) => { const n = [...prev]; n[i] = { ...n[i], category: e.target.value as 'internal' | 'external' }; return n; })} className="px-2 py-1.5 text-sm border rounded">
                              <option value="internal">Internal</option>
                              <option value="external">External</option>
                            </select>
                            <input type="text" value={r.type} onChange={(e) => setEventResources((prev) => { const n = [...prev]; n[i] = { ...n[i], type: e.target.value }; return n; })} placeholder="Type (e.g. Auditorium)" className="w-32 px-2 py-1.5 text-sm border rounded" />
                            <input type="text" value={r.description} onChange={(e) => setEventResources((prev) => { const n = [...prev]; n[i] = { ...n[i], description: e.target.value }; return n; })} placeholder="Description" className="flex-1 min-w-[120px] px-2 py-1.5 text-sm border rounded" />
                            <input type="number" min={0} value={r.estimatedCost ?? ''} onChange={(e) => setEventResources((prev) => { const n = [...prev]; n[i] = { ...n[i], estimatedCost: e.target.value ? Number(e.target.value) : undefined }; return n; })} placeholder="Est. cost (₹)" className="w-24 px-2 py-1.5 text-sm border rounded" />
                            <button type="button" onClick={() => setEventResources((prev) => prev.filter((_, j) => j !== i))} className="p-1.5 text-red-500 hover:bg-red-50 rounded"><Trash2 className="w-4 h-4" /></button>
                          </div>
                        ))}
                        <button type="button" onClick={() => setEventResources((prev) => [...prev, { category: 'internal' as const, type: '', description: '' }])} className="text-sm text-sgt-600 hover:text-sgt-700 font-medium flex items-center gap-1"><Plus className="w-4 h-4" /> Add resource</button>
                      </div>
                    )}
                  </div>
                </div>
