...
const EVENT_TYPE_COLORS: Record<string, string> = {
  ...
  workshop: 'bg-ev-100 text-ev-800 dark:bg-ev-900/30 dark:text-ev-200',
...
const STATUS_CONFIG: Record<string, { label: string; color: string; dot: string }> = {
  ...
  published: { label: 'Published', color: 'text-ev-700 dark:text-ev-400', dot: 'bg-ev-700' },
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] dark:bg-gray-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <div className="flex items-center gap-3 mb-2">
                <div className="p-2.5 bg-gradient-to-br from-ev-700 to-ev-800 rounded-xl">
                  <Shield className="h-6 w-6 text-white" />
                </div>
                <h1 className="text-3xl font-bold text-ev-900 dark:text-white">Volunteer Dashboard</h1>
              </div>
            ...
            <Link
              ...
              className="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-ev-700 to-ev-800 text-white rounded-lg hover:from-ev-800 hover:to-ev-800 transition-all font-medium shadow-ev"
            >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          {[
            { label: 'Total Assigned', value: stats.total, icon: Shield, color: 'from-ev-700 to-ev-800' },
            ...
            <div key={stat.label} className={CARD + ' p-4'}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-2xl font-bold text-ev-900 dark:text-white">{stat.value}</p>
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-6">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-ev-900 dark:text-white focus:ring-2 focus:ring-ev-600 focus:border-transparent"
            />
          </div>
          <select
            value={statusFilter}
            onChange={(e) => setStatusFilter(e.target.value)}
            className="px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-ev-900 dark:text-white"
          >
   ...
   0 ? (
              <>
                ...
                <Link
                  href="/events"
                  className="inline-flex items-center gap-2 px-5 py-2.5 bg-ev-700 text-white rounded-lg hover:bg-ev-800 transition"
                >
                  Browse Events
                </Link>
              </>
              ...
              <>
                ...
                <h3 className="text-lg font-semibold text-ev-900 dark:text-white mb-1">
   ...
   'ongoing';
              ...
              return (
                <Link
                  ...
                  className={`${CARD} hover:shadow-ev-lg hover:-translate-y-1 transition-all duration-200 overflow-hidden group`}
                >
                  {/* Color bar */}
                  <div className={`h-1.5 w-full ${isLive ? 'bg-gradient-to-r from-green-500 to-emerald-500' : isUpcoming ? 'bg-gradient-to-r from-ev-700 to-ev-700' : 'bg-gradient-to-r from-gray-400 to-gray-500'}`} />
                  ...
                  <div className="p-5">
                    ...
                    {/* Event Name */}
                    <h3 className="text-lg font-bold text-ev-900 dark:text-white mb-3 line-clamp-2 group-hover:text-ev-700 dark:group-hover:text-ev-400 transition-colors">
                    ...
                    {/* Volunteer Info */}
                    <div className="pt-3 border-t border-[#b3cde0] dark:border-gray-700">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          {assignment.role && (
                            <span className="inline-flex items-center gap-1 px-2 py-0.5 text-xs font-medium bg-ev-100 dark:bg-ev-900/30 text-ev-800 dark:text-ev-200 rounded-full">
                        ...
                        <ChevronRight className="h-5 w-5 text-gray-400 group-hover:text-ev-700 dark:group-hover:text-ev-400 transition-colors" />
                      </div>
